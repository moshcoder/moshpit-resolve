...
export function parseRegistryName(hostname) {
  ...
  // and first come first served, so there is nowhere to appeal it. Matching the
  // registry matters more than the rule itself: a name this accepts and the
...
function destinationFromDecision(decision, registryBase) {
...
/**
 * The whole policy for one navigation, including the evidence and explanation.
 *
 * This is the integration API for callers that need more than a URL: a browser
 * can navigate to `destination`, show `decision.reason` in its UI, and retain
 * the registry answer for diagnostics without making the lookup twice. The
 * shape is stable even when the input is not a Moshpit name or the registry is
 * unreachable.
 *
 * `clearnetResolves` is supplied by the caller because only it knows whether
 * DNS actually answered. Config is passed in rather than discovered so the
 * same function works in an extension, a server and a test.
 */
export async function resolutionFor(hostname, clearnetResolves, config = {}) {
  const parsed = parseRegistryName(hostname);
  if (!parsed) {
    return {
      registry: null,
  ...
  const {
    ...
    consoleBase = DEFAULT_CONSOLE_BASE,
    parkingBase = DEFAULT_PARKING_BASE,
  ...
  const isConsole = !!consoleUrlFor(name, consoleBase);
  const registry = isConsole ? null : await lookupMoshpit(name, { registryBase, timeoutMs });
  const decision = decideResolution({
    hostname: name, mode, clearnetResolves, moshpit: registry, consoleBase, parkingBase,
  });

  return {
    registry,
    decision,
    destination: destinationFromDecision(decision, registryBase),
  };
}

/**
 * The whole policy for one navigation, as a URL to send the tab to (or null to
 * leave it alone). Use `resolutionFor` when the caller also needs the registry
 * answer and the reason for the decision.
 */
export async function destinationFor(hostname, clearnetResolves, config = {}) {
  return (await resolutionFor(hostname, clearnetResolves, config)).destination;
}
