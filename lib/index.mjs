...
export async function destinationFor(hostname, clearnetResolves, config = {}) {
  ...
  // Config is passed in rather than read. Where it lives is the caller's
  // problem — chrome.storage in an extension, a file on a server, a flag in a
  // test — and a library that reaches for one of those only works in one of
  // those places.
  const {
    mode = 'clearnet',
